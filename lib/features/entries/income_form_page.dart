...
/// Formulaire de création / modification d'un revenu.
/// Passer [existing] pour éditer un revenu existant, sinon un nouveau revenu
/// est créé sur le cycle [cycleId].
class IncomeFormPage extends ConsumerStatefulWidget {
  final int cycleId;
  final IncomeEntity? existing;

  const IncomeFormPage({super.key, required this.cycleId, this.existing});
...
class _IncomeFormPageState extends ConsumerState<IncomeFormPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(_isEditing ? 'Modifier le revenu' : 'Nouveau revenu')),
      body: SafeArea(
        child: Form(
          key: _formKey,
          child: ListView(
            ...
            children: [
              ...
              SwitchListTile(
                title: const Text('Revenu récurrent'),
                value: _isRecurring,
